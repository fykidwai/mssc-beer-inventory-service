...
public class AllocationServiceImpl implements AllocationService {
    ...
    public void deallocateOrder(final BeerOrderDto beerOrderDto) {
        beerOrderDto.getBeerOrderLines().forEach(beerOrderLineDto -> {
            final BeerInventory beerInventory = BeerInventory.builder().beerId(beerOrderLineDto.getBeerId())
                .upc(beerOrderLineDto.getUpc()).quantityOnHand(beerOrderLineDto.getQuantityAllocated()).build();
            final BeerInventory savedBeerInventory = beerInventoryRepository.save(beerInventory);
            log.debug("Saved inventory for beer upc: " + savedBeerInventory.getUpc() + " inventory id: "
                + savedBeerInventory.getId());
        });
    ...
    private void allocateBeerOrderLine(final BeerOrderLineDto beerOrderLine) {
        ...
        beerInventoryList.forEach(beerInventory -> {
            ...
            if (beerInventory.getQuantityOnHand() == MN_0) {
